...
if [ "$#" -lt 6 ]; then
    ...
    echo "Usage: ./run_champsim.sh [BINARY] [N_WARM] [N_SIM] [TRACE_DIR_IN_TRACER] [TRACE] [OUTPUT_DIR] [OPTION]"
    exit 1
fi
...
mkdir -p ${OUTPUT_DIR}_${N_SIM}M
(./bin/${BINARY} -warmup_instructions ${N_WARM}000000 -simulation_instructions ${N_SIM}000000 ${OPTION} -traces ${TRACE_DIR}/${TRACE}) &> ${OUTPUT_DIR}_${N_SIM}M/${TRACE}-${BINARY}${OPTION}.txt
